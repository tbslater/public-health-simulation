...
The original model code can be found on GitHub and is freely available under the GPL-3.0 licence.

::: {.callout-note icon="false"}
## Source Code

Huang, S. W., J. Maingard, H. K. Kok, C. D. Barras, V. Thijs, R. V. Chandra, D. M. Brooks, and H. Asadi. 2019. <https://github.com/shiweih/desECR.git> \[Last accessed: 13/03/2025\].
:::

## INTRODUCTION

Reproducible research is critical. Publishing work that can be understood, replicated and developed by others is key to accelerating growth in any field. Yet, research as a whole is not reproducible, perhaps alarmingly so , and, in the interest of this article, modelling and simulation is certainly no exception to this.
...
DES is a type of simulation model, typically used in operational research. It models arrivals, exits and internal movements within a system as a network of entities, resources and queues. Individual entities may enter the system, leave the system, occupy a resource or queue for a resource at any given time.

A fundamental property of DES is that changes to a system can only occur when events are triggered. Thus, time progession is directly dictated by discrete events (hence, *discrete-event* simulation). Several discrete events may occur at once and so a key area of interest is the order in which DES deals with these.
...
![Discrete-event simulation system diagram - figure 1 from Huang et al. 2019](figures/huang_fig1_original.jpg){fig-align="center" width="60%"}
...
2.  **Exclusive-use:** elective IR patients can only use the IR angiography machine.
...
The R package `simmer` is used for simulation. The package is designed for DES model implementation and documentation can be found at [https://cran.r-project.org/web/packages/simmer/index.html](#0){.uri} [@RN9].

In order for a reproducibility analysis to be performed in the first place, open code must be provided and this has been done by the authors via GitHub (found at <https://github.com/shiweih/desECR>). Huang et al. ought to be commended for doing this, as this in itself drastically increases the reproducibility of their work. However, this is not necessarily enough; code that is not readable or fails to run is not of much use. Fortunately, I did not find this to be the case. Though additional comments may have been useful, reasonable knowledge of `simmer` and DES theory is sufficient for comprehension. The function used to run the simulation worked fine and was easily adaptable. A minor criticism is that, to me at least, the purpose of the plotting function was unclear, as none of the plots were relevant to those in the paper. So my own code has removed this and created plotting functions from scratch.
...
![Discrete-event simulation system diagram - figure 1 from Huang et al. 2019 ](figures/huang_fig2_original.jpg){fig-align="center" width="70%"}
...
![Discrete-event simulation system diagram - figure 1 from Huang et al. 2019 ](figures/huang_fig3_original.jpg){fig-align="center" width="70%"}
...
![Discrete-event simulation system diagram - figure 1 from Huang et al. 2019 ](figures/huang_fig4_original.jpg){fig-align="center" width="70%"}
...
![Discrete-event simulation system diagram - figure 1 from Huang et al. 2019 ](figures/huang_fig5_original.jpg){fig-align="center" width="70%"}
...
![Discrete-event simulation system diagram - figure 1 from Huang et al. 2019 ](figures/huang_suppfig_original.png){fig-align="center" width="70%"}
